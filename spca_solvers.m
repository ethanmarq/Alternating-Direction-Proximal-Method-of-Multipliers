...
algs = {'NS-ADPMM','SVD-ADPMM','RADMM','ARADMM','OADMM'}; % NON ManPG varaint
...
Tc = {T_adpmm(1:iter_adpmm), T_adpmm_svd(1:iter_adpmm_svd), ... % NON ManPG varaint
      T_radmm(1:iter_radmm), T_aradmm(1:iter_aradmm), T_oadmm(1:iter_oadmm)};
Fc = {F_adpmm(1:iter_adpmm), F_adpmm_svd(1:iter_adpmm_svd), ... % NON ManPG varaint
      F_radmm(1:iter_radmm), F_aradmm(1:iter_aradmm), F_oadmm(1:iter_oadmm)};
...
%       'FontSize', 20);
% legend(algs);
% fname = sprintf('spca_%s_n%d_p%d_mu%.2f_log_%s.png', dataset, n, p, mu, xtag);
% % END === Log(F* - F) by xtag
...
title(sprintf('sPCA %s (n=%d, k=%d, \\lambda=%g)', ds_disp, n, p, mu), ...
      'FontSize', 20);
legend(algs);
fname = sprintf('spca_%s_n%d_p%d_mu%.2f_%s.png', dataset, n, p, mu, xtag);
% END === F by xtag
